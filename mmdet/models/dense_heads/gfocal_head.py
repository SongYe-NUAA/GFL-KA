...
class GFocalHead(AnchorHead):
    ...
    def __init__(self,
                 ...
                 log_gt_top5_positive=False,  # 🎯 是否记录GT Top5正样本日志
    ...
    def _extract_enhanced_statistics(self, bbox_pred_fp32):
        ...
        top1_value = top4_values[:, :, 0:1, :, :]
        top2_value = top4_values[:, :, 1:2, :, :]
        top3_value = top4_values[:, :, 2:3, :, :]
        top4_value = top4_values[:, :, 3:4, :, :]
        ...
        if self.use_top3_plus_top4:
            ...
            original_features = torch.cat([top1_value,top2_value, top3_plus_top4,mean_feature], dim=2)
        else:
            # [top1, top2, top3, top4, mean] = [N, 4, 5, H, W]
            original_features = torch.cat([top2_value, top3_top4, mean_feature], dim=2)
