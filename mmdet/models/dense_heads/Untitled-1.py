...
class GFocalHead(AnchorHead):
    ...
    def _init_kurtosis_parameters(self):
        ...
        # === 🎯 改进1：可学习的通道重要性选择器 ===
        # 根据分析：TOP1(强相关)、Mean(强相关)、TOP2(中等)、TOP3/4(无相关)
        # 为每个通道添加可学习的缩放因子，让网络自动学习哪些特征更重要
        # total_dim = 4 (use_top3_plus_top4=True, add_mean=True)
        #           或 3 (use_top3_plus_top4=True, add_mean=False)
        #           或 5 (use_top3_plus_top4=False, add_mean=True)
        #           或 4 (use_top3_plus_top4=False, add_mean=False)
        self.channel_importance = nn.Parameter(torch.ones(self.total_dim))

        # === 🎯 改进2：方向间的重要性（4个方向可能有不同的权重） ===
        self.direction_importance = nn.Parameter(torch.ones(4))

        # === 🎯 改进3：Stride自适应归一化参数 ===
        # 用于减少不同stride之间的分布差异
        self.stride_adaptive_scale = nn.Parameter(torch.tensor(1.0))
        self.stride_adaptive_bias = nn.Parameter(torch.tensor(0.0))

        # === 🎯 训练监控计数器 ===
        self._importance_log_counter = 0
    ...
    def _log_importance_parameters(self, channel_importance_norm, direction_importance_norm):
        """🎯 训练监控：打印通道和方向重要性参数

        Args:
            channel_importance_norm: softmax归一化后的通道重要性权重 [total_dim]
            direction_importance_norm: softmax归一化后的方向重要性权重 [4]
        """
        self._importance_log_counter += 1

        # 每20个batch打印一次，避免日志过多
        if self._importance_log_counter % 20 != 0:
            return
        ...
        try:
            ...
            # 定义特征名称
            if self.use_top3_plus_top4:
                channel_names = ['Top1', 'Top2', 'Top3+4', 'Mean']
            else:
                channel_names = ['Top1', 'Top2', 'Top3', 'Top4', 'Mean']
            direction_names = ['Left', 'Top', 'Right', 'Bottom']

            # 打印表头
            print(f"\n{'='*80}")
            print(f"📊 通道/方向重要性监控 - Epoch {current_epoch}, Step {self._importance_log_counter}")
            print(f"{'='*80}")

            # 打印通道重要性
            print(f"\n🎯 通道重要性权重 (Softmax归一化):")
            channel_str = "   "
            for i, name in enumerate(channel_names[:len(channel_importance_norm)]):
                val = channel_importance_norm[i].item()
                channel_str += f"{name}: {val:.4f}  "
            print(channel_str)
            ...
            for i, name in enumerate(direction_names):
                val = direction_importance_norm[i].item()
                dir_str += f"{name}: {val:.4f}  "
            print(dir_str)

            # 打印原始未归一化的参数值
            print(f"\n🔧 原始参数值 (未经Softmax):")
            print(f"   channel_importance: {[f'{x:.4f}' for x in self.channel_importance.data.tolist()]}")
            print(f"   direction_importance: {[f'{x:.4f}' for x in self.direction_importance.data.tolist()]}")

            # 打印Stride自适应参数
            print(f"\n⚙️  Stride自适应参数:")
            print(f"   scale: {self.stride_adaptive_scale.item():.4f}")
            print(f"   bias: {self.stride_adaptive_bias.item():.4f}")

            # 打印峰度参数
            print(f"\n🔥 峰度加权参数:")
            print(f"   kurtosis_scale_factor: {self.kurtosis_scale_factor.item():.4f}")
            print(f"   kurtosis_bias: {self.kurtosis_bias.item():.4f}")
            print(f"   kurtosis_fusion_weight: {self.kurtosis_fusion_weight.item():.4f}")

            print(f"{'='*80}\n")

        except Exception as e:
            # 日志记录失败不应该影响训练
            print(f"⚠️ 重要性参数日志记录失败: {e}")

    def _extract_enhanced_statistics(self, bbox_pred_fp32, stride=None):
        ...
        - 🚀 支持Stride自适应归一化，减少不同层级间的分布差异

        Args:
            bbox_pred_fp32 (Tensor): 边界框预测 [N, 4*(reg_max+1), H, W]
            stride (int, optional): 当前特征层的stride，用于自适应归一化
        ...
        # === 🎯 改进1：应用通道重要性选择器 ===
        # 根据分析：TOP1(强相关)、Mean(强相关)、TOP2(中等)、TOP3/4(无相关)
        # 通过softmax归一化让网络学习最优的通道权重组合
        channel_importance_norm = F.softmax(self.channel_importance, dim=0)
        importance_per_dir = channel_importance_norm.view(1, 1, -1, 1, 1)  # [1, 1, total_dim, 1, 1]
        original_features = original_features * importance_per_dir

        # === 🎯 改进2：应用方向间重要性 ===
        # 4个方向可能有不同的整体权重
        direction_importance_norm = F.softmax(self.direction_importance, dim=0)  # [4]
        direction_importance_per_dir = direction_importance_norm.view(1, 4, 1, 1, 1)  # [1, 4, 1, 1, 1]
        original_features = original_features * direction_importance_per_dir

        # === 🎯 训练监控：打印通道和方向重要性参数 ===
        if self.training:
            self._log_importance_parameters(channel_importance_norm, direction_importance_norm)
        ...
        if self.use_kurtosis_weight:
            # 🎯 峰度加权前：分离TOP1以保护其与IOU的正相关性
            # 将特征按通道维度分离：TOP1 | (TOP2, TOP3+4, Mean)
            # original_features: [N, 4, total_dim, H, W]
            # total_dim = 4 时: [N, 4, 4, H, W] → TOP1:[N,4,1], 其余:[N,4,3]
            # total_dim = 5 时: [N, 4, 5, H, W] → TOP1:[N,4,1], 其余:[N,4,4]

            # 提取TOP1特征（第一个通道，保持与IOU的正相关）
            top1_features = original_features[:, :, 0:1, :, :]  # [N, 4, 1, H, W]

            # 提取其余特征（TOP2, TOP3+4/Mean）
            if self.use_top3_plus_top4:
                other_features = original_features[:, :, 1:, :, :]  # [N, 4, 3, H, W]
            else:
                other_features = original_features[:, :, 1:, :, :]  # [N, 4, 4, H, W]

            # 🎯 对TOP1应用正向增强（不受峰度因子符号影响）
            # 确保TOP1始终与IOU保持正相关：只增强，不抑制
            top1_enhanced = top1_features * (1 + kurtosis_expanded * torch.abs(self.kurtosis_fusion_weight) * 0.5 + 0.5)

            # 🎯 对其余特征应用峰度加权（可正可负，灵活调整）
            other_reshaped = other_features.reshape(N, -1, H, W)  # [N, 3/4*4, H, W]
            other_enhanced = other_reshaped * (1 + kurtosis_expanded * self.kurtosis_fusion_weight)

            # 拼接回合并特征 [N, (1+3/4)*4, H, W]
            enhanced_stat = torch.cat([top1_enhanced, other_enhanced], dim=1)
        else:
            # 🎯 不使用峰度加权：使用原始特征
            enhanced_stat = original_features.reshape(N, -1, H, W)

        # === 🎯 改进3：Stride自适应归一化 ===
        # 根据用户观察：S1287的低IOU的TOP1值可以赶上S8的高IOU值
        # 不同stride的特征分布存在显著差异，需要进行归一化
        # 使用 log(stride) 作为归一化因子，因为stride呈指数增长
        if stride is not None:
            log_stride = torch.log(torch.tensor(stride, dtype=torch.float32, device=enhanced_stat.device))
            # 归一化因子：scale * log(stride) + bias
            stride_norm_factor = self.stride_adaptive_scale * log_stride + self.stride_adaptive_bias
            # 应用归一化：除以stride归一化因子
            enhanced_stat = enhanced_stat / (stride_norm_factor + 1e-8)
    ...
    def forward_single(self, x, scale):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            # 获取当前层的stride
            stride = self.prior_generator.strides[self.current_level_idx]

            # 🚀 使用增强版峰度加权特征提取（4维训练自适应特征：top1, top2, top3+top4, mean）
            stat = self._extract_enhanced_statistics(bbox_pred_fp32, stride)
        ...
        if self.training:
            output = (cls_score_fp32.to(input_dtype), bbox_pred.to(input_dtype), quality_score.to(input_dtype))
        else:
            # 测试时返回FP32，避免不同GPU架构的FP16精度差异
            output = (cls_score_fp32, bbox_pred, quality_score)

        # 递增层级索引，供下一个forward_single调用使用
