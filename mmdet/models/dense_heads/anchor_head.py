...
class AnchorHead(BaseDenseHead):
    ...
    def __init__(
        ...
        feat_channels: int = 256,# 特征通道数，用于中间层的处理
        anchor_generator: ConfigType = dict(
            type='AnchorGenerator', # 锚框生成器类型
            scales=[8, 16, 32],# 锚框的尺度，相对于基础尺寸的倍数
            ratios=[0.5, 1.0, 2.0], # 锚框的长宽比
            strides=[4, 8, 16, 32, 64]), # 不同特征层级的步长，决定锚框的密度
        bbox_coder: ConfigType = dict(
            type='DeltaXYWHBBoxCoder', # 边界框编码器类型
            clip_border=True,# 是否裁剪边界框到图像边界
            target_means=(.0, .0, .0, .0), # 目标均值，用于边界框编码的归一化
            target_stds=(1.0, 1.0, 1.0, 1.0)),# 目标标准差，用于边界框编码的归一化
        reg_decoded_bbox: bool = False,# 是否在解码后的边界框上直接应用回归损失
        loss_cls: ConfigType = dict(
            type='CrossEntropyLoss', # 分类损失类型
            use_sigmoid=True, # 是否使用sigmoid激活函数
            loss_weight=1.0),# 分类损失的权重
        loss_bbox: ConfigType = dict(
            type='SmoothL1Loss',  # 边界框回归损失类型
            beta=1.0 / 9.0,# SmoothL1Loss的beta参数
            loss_weight=1.0),# 回归损失的权重
        train_cfg: OptConfigType = None, # 训练时的配置，包含正负样本定义等
        test_cfg: OptConfigType = None,# 测试时的配置，包含NMS等后处理参数
        init_cfg: OptMultiConfig = dict(
            type='Normal',  # 初始化方法类型
            layer='Conv2d',# 应用初始化的层类型
            std=0.01)# 正态分布的标准差
    ) -> None:
        super().__init__(init_cfg=init_cfg) # 调用父类的初始化方法
        self.in_channels = in_channels # 设置输入通道数
        self.num_classes = num_classes# 设置类别数量
        self.feat_channels = feat_channels # 保存特征通道数
        self.use_sigmoid_cls = loss_cls.get('use_sigmoid', False) # 根据是否使用sigmoid确定分类输出通道数
        if self.use_sigmoid_cls:
            self.cls_out_channels = num_classes# sigmoid时输出通道数等于类别数
        else:
            self.cls_out_channels = num_classes + 1# softmax时需要额外的背景类
        # 检查类别数是否合法
        if self.cls_out_channels <= 0:
            raise ValueError(f'num_classes={num_classes} is too small')
        self.reg_decoded_bbox = reg_decoded_bbox# 是否在解码后的bbox上计算回归损失

        self.bbox_coder = TASK_UTILS.build(bbox_coder)  # 构建边界框编码器
        self.loss_cls = MODELS.build(loss_cls)# 构建分类损失函数,使用MMDET的注册器机制构建损失函数对象
        self.loss_bbox = MODELS.build(loss_bbox)# 构建回归损失函数
        self.train_cfg = train_cfg# 保存训练配置
        self.test_cfg = test_cfg # 保存测试配置
        # 如果有训练配置,构建assigner和sampler
        if self.train_cfg:
            self.assigner = TASK_UTILS.build(self.train_cfg['assigner'])
            if train_cfg.get('sampler', None) is not None:
                # 如果指定了sampler,则构建对应的sampler
        ...
        self.fp16_enabled = False # 是否启用FP16训练

        self.prior_generator = TASK_UTILS.build(anchor_generator)   # 构建锚框(prior)生成器

        # Usually the numbers of anchors for each level are the same
        # except SSD detectors. So it is an int in the most dense
        # heads but a list of int in SSDHead
        # 获取每个特征层的基础锚框数量
        # 通常每个特征层的锚框数量相同(除了SSD),所以取第一个值即可
