...
class BaseDenseHead(BaseModule, metaclass=ABCMeta):
    ...
    def loss(self, x: Tuple[Tensor], batch_data_samples: SampleList) -> dict:
        ...
        """
        outs = self(x)# outs通常包含分类分数(cls_scores)和边界框预测(bbox_preds)
        # 2. 从batch_data_samples中解包ground truth数据
        outputs = unpack_gt_instances(batch_data_samples)
        (batch_gt_instances,# 真实标注框和类别标签
         batch_gt_instances_ignore,# 需要忽略的标注框(如果有)
         batch_img_metas) = outputs# 图像的元信息(如尺寸、缩放因子等)
        # 3. 组合模型输出和ground truth用于计算损失
        loss_inputs = outs + (batch_gt_instances, batch_img_metas,
                              batch_gt_instances_ignore)
        # loss_inputs实际上是一个元组，包含:
        # - cls_scores: 分类预测
        # - bbox_preds: 边界框预测
        # - batch_gt_instances: ground truth标注
        # - batch_img_metas: 图像信息
        # - batch_gt_instances_ignore: 忽略的标注(可选)

        # 4. 计算损失
        losses = self.loss_by_feat(*loss_inputs)  # 展开loss_inputs元组作为参数传入
        return losses # 返回计算得到的损失字典
