...
class iFormer(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000,
        ...
        super().__init__()
        self.init_cfg = init_cfg  # 保存初始化配置
        self.use_bn = use_bn
    ...
    def init_weights(self):
        ...
        if isinstance(self.init_cfg, dict) and self.init_cfg.get('type') == 'Pretrained':
            ...
            try:
                ...
                # 检查是否是检查点格式(包含'model', 'optimizer'等键)
                if 'model' in ckpt:
                    logger.info('Found checkpoint format, loading model state_dict')
                    # 只加载权重部分
                    state_dict = ckpt['model']
                elif isinstance(ckpt, dict) and 'state_dict' in ckpt:
                    logger.info('Found state_dict format, loading state_dict')
                ...
                # 移除head/classifier相关的参数
                keys_to_remove = []
                for k in new_state_dict.keys():
                    if k.startswith('head.') or k.startswith('classifier.') or k.startswith('norm.'):
                        keys_to_remove.append(k)
                
                for k in keys_to_remove:
                    new_state_dict.pop(k, None)
                
                # 使用严格=False加载，允许缺失某些键
                ...
                logger.warning('Initializing from scratch')
                self.apply(self._init_weights)
        else:
            # 默认初始化
            self.apply(self._init_weights)
...
@register_model
@MODELS.register_module()
def iFormer_t(pretrained=False, in_22k=False, **kwargs):
    depths = [2, 2, 16, 6]
...
@register_model
@MODELS.register_module()
def iFormer_s(pretrained=False, in_22k=False, **kwargs):
    depths = [2, 2, 19, 6]
...
@register_model
@MODELS.register_module()
def iFormer_m(pretrained=False, in_22k=False, **kwargs):
    depths = [2, 2, 22, 6]
    block_types = ['ConvBlock_k7_r4'] * 2 + ['ConvBlock_k7_r4'] * 2 + ['ConvBlock_k7_r4'] * 9 + \
                  ['RepCPE_k3', 'SHMABlock_r1_hdrr2_act0_nh1', 'FFN2d_r3'] * 4 + ['ConvBlock_k7_r4'] + \
                  ['RepCPE_k3', 'SHMABlock_r1_hdrr4_act0_nh1', 'FFN2d_r3'] * 2
    
    # 处理init_cfg参数，避免传递给iFormer类
    init_cfg = kwargs.pop('init_cfg', None)
...
@register_model
@MODELS.register_module()
def iFormer_h(pretrained=False, in_22k=False, **kwargs):
    depths = [5, 5, 60, 18]
