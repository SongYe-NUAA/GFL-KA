...
class WiseIoULoss(nn.Module):
    r"""Wise-IoU Loss for Bounding Box Regression.

    Implementation of Wise-IoU with three versions:
    - v1: Basic version with IoU-based attention
    - v2: Version with outlier-emphasized attention
    - v3: Version with dynamic non-monotonic focusing strategy

    Args:
        version (str): Version of Wise-IoU, options are 'v1', 'v2', 'v3'.
            Default: 'v3'
        beta (float): Beta parameter for Wise-IoU. Default: 1.0
        eps (float): Epsilon to avoid log(0). Default: 1e-7
    ...
    def forward(self,
        ...
        Args:
            ...
            target (Tensor): The learning target of the prediction,
                shape (n, 4).
            iou (Tensor, optional): IoU scores for each bbox pair.
                Shape (n,). Used for dynamic weighting in Wise-IoU.
            weight (Optional[Tensor], optional): The weight of loss for each
                prediction. Defaults to None.
            avg_factor (Optional[int], optional): Average factor that is used
                to average the loss. Defaults to None.
        ...
        iou_loss_value = 1 - ious

        # Apply Wise-IoU weighting based on version
        if iou is not None and self.version in ['v1', 'v2', 'v3']:
            # Clamp IoU to avoid extreme values
            iou_clamped = iou.clamp(min=self.eps, max=1.0 - self.eps)

            if self.version == 'v1':
                # Basic version: focal attention based on IoU
                attention_weight = iou_clamped.pow(2)
                loss = iou_loss_value * attention_weight

            elif self.version == 'v2':
                # Version with outlier emphasis
                dist = torch.abs(ious - 1.0)
                outlier_factor = dist.pow(self.beta)
                attention_weight = iou_clamped / (outlier_factor + self.eps)
                loss = iou_loss_value * attention_weight

            elif self.version == 'v3':
                # Version with dynamic non-monotonic focusing strategy
                # r = exp((1 - IoU)^2 / sigma) for sigma = 7
                sigma = 7.0
                r = torch.exp((1 - iou_clamped.pow(2)) / sigma)

                # For low quality samples (IoU < threshold): increase weight
                # For high quality samples (IoU >= threshold): decrease weight
                with torch.no_grad():
                    if self.beta > 0:
                        # Dynamic threshold based on beta
                        threshold = min(ious.mean() + self.beta * ious.std(), 0.7)
                    else:
                        threshold = 0.7

                    # Create attention weight with focusing
                    weight_map = torch.ones_like(ious)
                    low_quality_mask = iou_clamped < threshold
                    if low_quality_mask.any():
                        # Increase weight for low quality samples
                        weight_map[low_quality_mask] = r[low_quality_mask]

                loss = iou_loss_value * weight_map

            else:
                loss = iou_loss_value

        else:
            # If no IoU provided, use standard IoU loss
            loss = iou_loss_value
        ...
        if reduction == 'mean':
            if avg_factor is not None and avg_factor > 0:
                ...
                loss = loss.sum() / weight.sum().clamp(min=self.eps)
            else:
                loss = loss.mean()
        elif reduction == 'sum':
            loss = loss.sum()
        # else: reduction == 'none', return raw loss
